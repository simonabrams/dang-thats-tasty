...
function autocomplete(input, latInput, lngInput) {
	if (!input) return; // skips this function if there's no input on the page
	...
	input.on('keydown', function (e) {
		if (e.keyCode == 13) e.preventDefault(); // 13 = 'enter' key
	});
}
...
function typeAhead(search) {
	...
	searchInput.on('input', function () {
		...
		//  if no value, hide the results and quit
		if (!this.value) {
	...
	searchInput.on('keyup', function (e) {
		// skip if they're not pressing up, down or enter
...
function loadPlaces(map) {
	...
	_axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
		...
		var markers = places.map(function (place) {
			var _place$loctation$coor = _slicedToArray(place.loctation.coordinates, 2),
			    placeLng = _place$loctation$coor[0],
			    placeLat = _place$loctation$coor[1];
